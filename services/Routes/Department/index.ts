...
import {
    DepartmentParser,
    DepartmentRequest,
} from "../BodyParserMiddleware/DepartmentParser";
...
router.get("/:departmentId", async (req, res) => {
    simpleFinalMWDecorator(res, async () => {
        ...
        const department = await logic.getDepartmentById(
...
router.put("/:departmentId", parser.completeParser, async (req, res) => {
    simpleFinalMWDecorator(res, async () => {
        const logic: DepartmentsLogic = new DepartmentsLogicImpl();
        const depReq = req as DepartmentRequest;
        const d = await logic.updateDepartment(
            req.params.departmentId,
            depReq.department
        );
        return d;
...
router.post("/:departmentId/professors", async (req, res) => {
    simpleFinalMWDecorator(
        ...
        async () => {
            const logic: DepartmentsLogic = new DepartmentsLogicImpl();
            await logic.addProfessorToDepartment(
                req.params.departmentId,
...
router.post("/:departmentId/courses", async (req, res) => {
    simpleFinalMWDecorator(res, async () => {
        const logic: DepartmentsLogic = new DepartmentsLogicImpl();
        await logic.addCourseToDepartment(
            req.params.departmentId,
            req.body.courseId
        );
    });
