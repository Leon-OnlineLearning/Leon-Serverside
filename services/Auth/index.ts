    ...
    new JWTStrategy(
        {
            jwtFromRequest: (req) => {
                ...
                if (req && (req.cookies || req.body["jwt"]))
                    token = req.cookies["jwt"] || req.body["jwt"];
                return token;
            },
    ...
    new GoogleStrategy(
        {
            ...
            clientSecret:
                process.env.GOOGLE_OAUTH2_CLIENT_SECRET || "TOP_SECRET",
...
export const BlockedJWTMiddleware = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    ...
    try {
        if (await isTokenBlocked(token)) res.status(401).send("Token Blocked!");
        else next();
    } catch (e) {
