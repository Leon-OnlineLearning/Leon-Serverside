...
export default async function populateDB() {
    ...
    let sample_professor = _createUser(new Professor(), "professor") as Professor
    await professorlogic.createProfessor(sample_professor);

    new DepartmentsLogicImpl().addProfessorToDepartment(sample_department.id, sample_professor.id)
    new DepartmentsLogicImpl().addCourseToDepartment(sample_department.id, sample_course.id)
